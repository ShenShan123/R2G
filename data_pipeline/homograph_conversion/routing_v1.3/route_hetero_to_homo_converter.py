...
A universal tool to convert PyTorch Geometric heterogeneous graphs (HeteroData) to homogeneous graphs (Data).

Features:
- Automatically handles different node and edge types
- Unifies feature dimensions
- Preserves labels and global features
- Supports custom mapping tables or automatic generation
...
class HeteroToHomoConverter:
    ...
    def __init__(self, target_feature_dim: int = 10):
        ...
        # Built-in mapping table - all node and edge types based on comprehensive analysis
        ...
        self.edge_type_to_id = {
            ...
            "('gate', 'has', 'pin')": 2,
            "('gate', 'has_pin', 'pin')": 2,  # Same as 'has', merged encoding [adjusted in new version]
    ...
    def _create_mapping(self, hetero_data: HeteroData):
        """Automatically create type mappingss"""
    ...
    def _normalize_features(self, features: torch.Tensor, target_dim: int = 10) -> torch.Tensor:
        """Unify feature dimensions to the specified dimension (default 10 dimensions)
        ...
        """
        # Ensure feature tensor is 2-dimensional
        if features.dim() == 1:
            features = features.unsqueeze(1)  # Convert 1D tensor to 2D
        elif features.dim() == 0:
            features = features.unsqueeze(0).unsqueeze(1)  # Convert 0D tensor to 2D
    ...
    def convert(self, hetero_data: HeteroData) -> Data:
        ...
        Args:
            hetero_data: Input heterogeneous graph data

        Returns:
            Data: Converted homogeneous graph data
        """
        # If no mapping tables exist, automatically create them
        ...
        # Note: Mapping information is no longer saved to .pt files
        # Mapping table information has been encoded in the last dimension of node and edge features
    ...
    def _convert_nodes(self, hetero_data: HeteroData) -> Tuple[torch.Tensor, Optional[torch.Tensor], Dict]:
        ...
        current_idx = 0
        has_labels = False
        max_label_dim = 1  # Record maximum label dimension

        # First pass: Determine maximum label dimension
        for node_type in hetero_data.node_types:
            ...
            if labels is not None:
                has_labels = True
                if labels.dim() > 1:
                    max_label_dim = max(max_label_dim, labels.shape[1])
        ...
        for node_type in hetero_data.node_types:
            ...
            labels = self._extract_labels(node_data)
            if labels is not None:
                # Ensure label dimensions are consistent
                if labels.dim() == 1 and max_label_dim > 1:
                    # Expand 1D label to 2D
                    labels = labels.unsqueeze(1)
                    if labels.shape[1] < max_label_dim:
                        padding = torch.zeros((labels.shape[0], max_label_dim - labels.shape[1]), dtype=labels.dtype)
                        labels = torch.cat([labels, padding], dim=1)
                elif labels.dim() > 1 and labels.shape[1] < max_label_dim:
                    # Pad to maximum dimension
                    padding = torch.zeros((labels.shape[0], max_label_dim - labels.shape[1]), dtype=labels.dtype)
                    labels = torch.cat([labels, padding], dim=1)
                all_node_labels.append(labels)
            else:
                # Create default labels, dimension matches maximum label dimension
                if max_label_dim > 1:
                    default_labels = torch.full((num_nodes, max_label_dim), -1, dtype=torch.long)
                else:
                    default_labels = torch.full((num_nodes,), -1, dtype=torch.long)
                all_node_labels.append(default_labels)

            # Establish node mapping
    ...
    def _convert_edges(self, hetero_data: HeteroData, node_mapping: Dict) -> Tuple[torch.Tensor, torch.Tensor, Optional[torch.Tensor]]:
        ...
        all_edge_labels = []
        has_labels = False
        max_edge_label_dim = 1  # Record maximum edge label dimension

        # First pass: Determine maximum edge label dimension
        for edge_type in hetero_data.edge_types:
            ...
            if labels is not None:
                has_labels = True
                if labels.dim() > 1:
                    max_edge_label_dim = max(max_edge_label_dim, labels.shape[1])
        ...
        for edge_type in hetero_data.edge_types:
            ...
            if labels is not None:
                ...
                # Ensure edge label dimensions are consistent
                if labels.dim() == 1 and max_edge_label_dim > 1:
                    # Expand 1D label to 2D
                    labels = labels.unsqueeze(1)
                    if labels.shape[1] < max_edge_label_dim:
                        padding = torch.zeros((labels.shape[0], max_edge_label_dim - labels.shape[1]), dtype=labels.dtype)
                        labels = torch.cat([labels, padding], dim=1)
                elif labels.dim() > 1 and labels.shape[1] < max_edge_label_dim:
                    # Pad to maximum dimension
                    padding = torch.zeros((labels.shape[0], max_edge_label_dim - labels.shape[1]), dtype=labels.dtype)
                    labels = torch.cat([labels, padding], dim=1)
                all_edge_labels.append(labels)
            else:
                # Create default edge labels, dimension matches maximum edge label dimension
                if max_edge_label_dim > 1:
                    default_labels = torch.full((len(new_src_indices), max_edge_label_dim), -1, dtype=torch.long)
                else:
                    default_labels = torch.full((len(new_src_indices),), -1, dtype=torch.long)
                all_edge_labels.append(default_labels)
...
def convert_file(input_file: str, output_file: str, converter: HeteroToHomoConverter):
    ...
    try:
        ...
        print(f"  Number of nodes: {homo_data.x.shape[0]}")
        print(f"  Number of edges: {homo_data.edge_index.shape[1]}")
...
def main():
    """Main function"""
    parser = argparse.ArgumentParser(description="Universal heterogeneous to homogeneous graph converter")
    parser.add_argument("input", help="Input heterogeneous graph file path or directory path")
    parser.add_argument("-o", "--output", help="Output homogeneous graph file path")
    ...
    if os.path.isfile(args.input):
        ...
        # Batch convert all .pt files in the directory
        success_count = 0
        total_count = 0
        ...
        print(f"Batch conversion complete: {success_count}/{total_count} files successfully converted")
