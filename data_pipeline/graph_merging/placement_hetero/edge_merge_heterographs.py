...
This script merges multiple heterogeneous graph files into one large graph, referencing advanced_graph_integration1.py design method:
- Generate graph_id for all nodes of each subgraph as identification flag for different graphs
- Preserve all global features (global_features, die_coordinates, etc.)
- Preserve all node and edge features as well as labels
- Ensure data integrity and correctness

Function description:
- Load all heterogeneous graph files in specified directory
- Add graph_id feature to all node types of each graph
- Merge all nodes and edges, maintaining continuity of node indices
- Preserve global features and label information
- Save merged large graph
...
def load_heterograph_files(input_dir: str) -> List[Tuple[str, HeteroData]]:
    ...
    heterograph_files = [f for f in pt_files if f.endswith('_D_heterograph.pt')]

    print(f"📁 Found {len(heterograph_files)} heterogeneous graph files")
    ...
    for file_path in sorted(heterograph_files):
        try:
            ...
            # Extract design name (remove _D_heterograph.pt suffix)
            design_name = os.path.basename(file_path).replace('_D_heterograph.pt', '')
...
def add_graph_id_to_all_nodes(data: HeteroData, graph_id: int) -> HeteroData:
    """Add subgraph ID graph_id feature to all node types of heterogeneous graph
    ...
    # Copy all node type data and add subgraph ID graph_id
    for node_type in data.node_types:
        if hasattr(data[node_type], 'x') and data[node_type].x is not None:
            original_features = data[node_type].x
            num_nodes = original_features.shape[0]

            # Create subgraph ID graph_id feature column (all nodes have same subgraph ID)
            graph_id_feature = torch.full((num_nodes, 1), graph_id, dtype=torch.float32)

            # Add graph_id feature to end of original features
    ...
    for edge_type in data.edge_types:
        ...
        if hasattr(data[edge_type], 'edge_label') and data[edge_type].edge_label is not None:
            ...
            print(f"  🏷️  Copied {edge_type} edge labels: {data[edge_type].edge_label.shape}")

    # Copy global features (if exist)
...
def merge_heterographs_advanced(graph_list: List[Tuple[str, HeteroData]]) -> HeteroData:
    ...
    Reference advanced_graph_integration1.py design method
    ...
    """
    print(f"🔗 Start merging {len(graph_list)} heterogeneous graphs...")

    if not graph_list:
        raise ValueError("No heterogeneous graphs to merge")
    ...
    for node_type in all_node_types:
        ...
        if node_features:
            ...
            print(f"  📊 {node_type}: {merged_graph[node_type].x.shape[0]} nodes, {merged_graph[node_type].x.shape[1]}D features")
    ...
    for edge_type in all_edge_types:
        ...
        for i, data in enumerate(processed_graphs):
            if edge_type in data.edge_types:
                ...
                # Process edge attributes (if exist)
                if hasattr(data[edge_type], 'edge_attr') and data[edge_type].edge_attr is not None:
                    edge_attrs.append(data[edge_type].edge_attr)

                # Process edge labels (if exist)
                if hasattr(data[edge_type], 'edge_label') and data[edge_type].edge_label is not None:
                    edge_labels.append(data[edge_type].edge_label)
...
def save_merged_graph(merged_graph: HeteroData, output_path: str):
    ...
    for node_type in sorted(merged_graph.node_types):
        if hasattr(merged_graph[node_type], 'x'):
            ...
            print(f"    * {node_type}: {count:,} nodes, {feature_dim}D features (including graph_id)")
...
def main():
    ...
    # Configure paths
    input_dir = "/Users/david/Desktop/place1.2/D_graph/batch_output"
    output_path = "/Users/david/Desktop/place1.2/D_graph/place_D_heterograph.pt"
    ...
    try:
        ...
        print("\n🎉 Heterogeneous graph merging completed!")
        print(f"📁 Output file: {output_path}")
        print(f"🔧 Improved features: graph_id generation, global features preservation, node labels preservation")
        return 0
