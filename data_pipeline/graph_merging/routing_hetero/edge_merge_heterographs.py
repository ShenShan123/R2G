#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""heterograph merge script - improved version

[CN][CN][CN][CN][CN][CN][CN][CN][CN][CN][CN][CN][CN][CN][CN][CN][CN],referenceadvanced_graph_integration1.py[CN][CN][CN][CN][CN]:
- [CN][CN][CN]graph's[CN][CN]nodegenerategraph_idas identification[CN][CN] for different graphs
- preserve allglobalfeature(global_features,die_coordinates[CN])
- preserve allnode[CN]edge[CN]featureand label
- ensure data integrity and correctness

[CN][CN][CN][CN]:
- load all heterograph files in specified directory
- [CN][CN]graph's[CN][CN]nodetype addgraph_idfeature
- [CN][CN][CN][CN]node[CN]edge,keepnode[CN][CN][CN][CN][CN][CN]
- Preserve global features[CN] labelinformation
...
def load_heterograph_files(input_dir: str) -> List[Tuple[str, HeteroData]]:
    ...
    """
    print(f"[SEARCH] Scanning directory: {input_dir}")
    ...
    heterograph_files = [f for f in pt_files if f.endswith('_f_heterograph.pt')]

    print(f"[FILE] Found {len(heterograph_files)} RF heterogeneous graph files")

    loaded_graphs = []
    for i, file_path in enumerate(sorted(heterograph_files)):
        try:
            print(f"📖 Loading: {os.path.basename(file_path)}")
            if i == 0:
                print(f"\n[SEARCH] First file: {os.path.basename(file_path)}")
            data = torch.load(file_path, map_location='cpu', weights_only=False)

            # Extract design name (remove _f_heterograph.pt suffix)
            design_name = os.path.basename(file_path).replace('_f_heterograph.pt', '')
    ...
    print(f"[OK] Successfully loaded {len(loaded_graphs)} heterogeneous graphs")
...
def add_graph_id_to_all_nodes(data: HeteroData, graph_id: int) -> HeteroData:
    ...
    for node_type in data.node_types:
        if hasattr(data[node_type], 'x') and data[node_type].x is not None:
            ...
            print(f"  [CHART] Added subgraph ID {graph_id} to {num_nodes} {node_type} nodes")
    ...
    for edge_type in data.edge_types:
        ...
        # Copy edge labels - prioritize checking edge_label
        if hasattr(data[edge_type], 'edge_label') and data[edge_type].edge_label is not None:
            new_data[edge_type].edge_label = data[edge_type].edge_label.clone()
            print(f"  🏷️  Copied {edge_type} edge labels (edge_label): {data[edge_type].edge_label.shape}")
        elif hasattr(data[edge_type], 'y') and data[edge_type].y is not None:
            new_data[edge_type].y = data[edge_type].y.clone()
            print(f"  🏷️  Copied {edge_type} edge labels (y): {data[edge_type].y.shape}")
    ...
    # copy graph-level labels([CN][CN][CN][CN])
    if hasattr(data, 'y') and data.y is not None:
        new_data.y = data.y.clone()
        print(f"  [TARGET] copy graph-level labels: {data.y.shape}")
...
def merge_heterographs_advanced(graph_list: List[Tuple[str, HeteroData]]) -> HeteroData:
    ...
    for node_type in all_node_types:
        ...
        if node_features:
            ...
            print(f"  [CHART] {node_type}: {merged_graph[node_type].x.shape[0]} nodes, {merged_graph[node_type].x.shape[1]} feature dimensions")
    ...
    for edge_type in all_edge_types:
        ...
        print(f"  [SEARCH] Processing edge type: {edge_type}")
        ...
        for i, data in enumerate(processed_graphs):
            if edge_type in data.edge_types:
                ...
                # Handle edge labels (if they exist) - prioritize checking edge_label
                edge_data = data[edge_type]
                has_edge_label = hasattr(edge_data, 'edge_label') and edge_data.edge_label is not None
                has_y_label = hasattr(edge_data, 'y') and edge_data.y is not None

                if i == 0:  # Only print debug information for the first graph
                    print(f"    📋 Graph {i}: edge_label exists={has_edge_label}, y exists={has_y_label}")
                    if has_edge_label:
                        print(f"    📋 edge_label shape: {edge_data.edge_label.shape}")
                    if has_y_label:
                        print(f"    📋 y shape: {edge_data.y.shape}")

                if has_edge_label:
                    edge_labels.append(edge_data.edge_label)
                elif has_y_label:
                    edge_labels.append(edge_data.y)
        ...
        if edge_indices:
            ...
            # Merge edge labels
            if edge_labels:
                merged_graph[edge_type].edge_label = torch.cat(edge_labels, dim=0)
                print(f"    Edge labels: {merged_graph[edge_type].edge_label.shape}")

                # Specifically mark the label preservation status for io_pin-pin and pin-pin edges
                if edge_type == ('io_pin', 'connects_to', 'pin'):
                    print(f"    [OK] io_pin-pin edge labels preserved: {merged_graph[edge_type].edge_label.shape}")
                elif edge_type == ('pin', 'connects_to', 'pin'):
                    print(f"    [OK] pin-pin edge labels preserved: {merged_graph[edge_type].edge_label.shape}")
    
    # Merge global features
    print("\n[GLOBE] Merging global features...")
...
def save_merged_graph(merged_graph: HeteroData, output_path: str):
    ...
    """
    print(f"[SAVE] Saving merged graph to: {output_path}")
    ...
    # Display detailed information for each edge type
    print(f"\n  [CHART] Edge type detailed information:")
    for edge_type in sorted(merged_graph.edge_types):
        edge_count = merged_graph[edge_type].edge_index.shape[1]
        print(f"    * {edge_type}: {edge_count:,} edges")

        # Display edge feature information
        if hasattr(merged_graph[edge_type], 'edge_attr') and merged_graph[edge_type].edge_attr is not None:
            edge_attr_dim = merged_graph[edge_type].edge_attr.shape[1]
            print(f"      - Edge features: {edge_attr_dim} dimensions")

        # Display edge label information
        if hasattr(merged_graph[edge_type], 'edge_label') and merged_graph[edge_type].edge_label is not None:
            edge_label_dim = merged_graph[edge_type].edge_label.shape[1]
            print(f"      - Edge labels: {edge_label_dim} dimensions [OK]")

            # Specifically mark label preservation for important edge types
            if edge_type == ('io_pin', 'connects_to', 'pin'):
                print(f"      - [TARGET] io_pin-pin edge labels fully preserved")
            elif edge_type == ('pin', 'connects_to', 'pin'):
                print(f"      - [TARGET] pin-pin edge labels fully preserved")
        elif hasattr(merged_graph[edge_type], 'y') and merged_graph[edge_type].y is not None:
            edge_label_dim = merged_graph[edge_type].y.shape[1]
            print(f"      - Edge labels (y): {edge_label_dim} dimensions [OK]")
        else:
            print(f"      - Edge labels: None")
...
def main():
    """Main function"""
    print("[START] Heterogeneous graph merging script started (improved version)")
    print("=" * 60)

    # Configure paths
    input_dir = "/Users/david/Desktop/route1.3/RF"
    output_path = "/Users/david/Desktop/route1.3/RF_merged_heterograph.pt"
    ...
    try:
        ...
        if not graph_list:
            print("[X] Error: No loadable heterogeneous graph files found")
            return 1
        ...
        print(f"[X] Error during merging: {e}")
        import traceback
        traceback.print_exc()
        return 1
