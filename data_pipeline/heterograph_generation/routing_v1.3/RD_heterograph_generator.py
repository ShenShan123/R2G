#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Routing[CN][CN]D[CN][CN][CN][CN]generate[CN](Pin-Pin connection[CN][CN])

[CN][CN][CN] based onrouting[CN][CN]DEF[CN][CN],[CN][CN]D[CN] convertFeatures:
1. [CN][CN]netnode[CN][CN][CN][CN]IO_Pin-Net,Pin-Netedge
2.  build[CN][CN]IO_Pin-Pin[CN]Pin-Pinedge,edge label[CN][CN]routing[CN][CN][CN]wire_length[CN]via_count
3. [CN][CN][CN][CN] outputpin([CN]ZN,Z,Q[CN])[CN][CN][CN][CN], connection[CN][CN][CN] inputpin
4.  parseroutinginformation, calculate[CN]net[CN] actual[CN][CN][CN][CN][CN][CN][CN]
5. global label[CN][CN]routing[CN][CN][CN][CN][CN][CN]

 convert[CN][CN]:
- [CN][CN][CN]net,[CN][CN][CN][CN][CN] outputpin([CN][CN][CN][CN][CN])
- [CN] outputpin connection[CN][CN]net[CN][CN][CN][CN] inputpin
- edge label[CN][CN]routing[CN][CN][CN][wire_length, via_count]
- global label[CN][CN][utilization, hpwl, total_wire_length, total_vias]

[CN][CN][CN][CN](compared toplacement[CN][CN]):
- edge label:[CN]HPWLchanged to[wire_length, via_count]
- global label:[CN][CN]total_wire_length[CN]total_vias
-  data[CN]:[CN]route_data_extract.csv readrouting[CN][CN][CN][CN] data

Author: EDA for AI Team
date: 2024

[CN][CN][CN][CN]:
    python RD_heterograph_generator.py input_route.def -o output.pt
"""
...
# ============================================================================
# DEF PARSER SECTION - DEF[CN][CN] parse[CN]region([CN][CN]B[CN][CN][CN][CN] parse[CN])
# ============================================================================
...
class DEFParser:
    ...
    def _parse_tracks(self, content: str):
        ...
        for match in re.finditer(track_pattern, content):
            ...
            # [CN][CN][CN]layer data structure([CN][CN][CN][CN][CN])
...
# ============================================================================
# FEATURE CONFIG SECTION - feature configurationregion([CN][CN]B[CN][CN][CN][CN] configuration)
# ============================================================================
...
# ============================================================================
# ENCODING UTILS SECTION -  encoding[CN][CN][CN]region([CN][CN]B[CN][CN][CN][CN] encoding[CN][CN])
# ============================================================================
...
# ============================================================================
# FEATURE ENGINEERING SECTION - feature[CN][CN]region([CN][CN]B[CN][CN][CN][CN]feature[CN][CN])
# ============================================================================
...
# ============================================================================
# D HETEROGRAPH BUILDER SECTION - D[CN][CN][CN][CN] build[CN]region
# ============================================================================

class DHeteroGraphBuilder:
    """Routing[CN][CN]D[CN][CN][CN][CN] build[CN]
    
     based onrouting[CN][CN]DEF[CN][CN],[CN][CN]D[CN] convertFeatures:
    1. [CN][CN]netnode[CN][CN][CN][CN]IO_Pin-Net,Pin-Netedge
    2.  build[CN][CN]IO_Pin-Pin[CN]Pin-Pinedge,edge label[CN][CN]routing[CN][CN][CN]wire_length[CN]via_count
    3. [CN][CN][CN][CN] outputpin[CN][CN][CN][CN], connection[CN][CN][CN] inputpin
    4.  parseroutinginformation, calculate[CN]net[CN] actual[CN][CN][CN][CN][CN][CN][CN]
    5. global label[CN][CN]routing[CN][CN][CN][CN][CN][CN](utilization,hpwl,total_wire_length,total_vias)
    
    nodetype: Gate, IO_Pin, Pin
    edgetype: IO_Pin-Pin, Pin-Pin, Gate-Pin
    edge label: [wire_length, via_count] (2[CN])
    global label: [utilization, hpwl, total_wire_length, total_vias] (4[CN])
    """
    ...
    def __init__(self, def_data: Dict, def_file_path: str = None):
        ...
        self.gate_name_to_idx = {}         # Gatenode mapping
    ...
    def _cache_net_features(self):
        ...
        for net_name, net_info in nets.items():
            #  calculatenetfeature: [net_type_id, connection_count] ([CN]B[CN] consistent)
    ...
    def _add_pin_pin_edges(self):
        ...
        for net_name, net_info in nets.items():
            ...
            # [CN][CN]net[CN] actual[CN][CN][CN][CN][CN][CN][CN][CN][CN] label
                ...
                for output_comp, output_pin_name, output_id in output_pins:
                    for input_comp, input_pin_name, input_id in input_pins:
                        if output_id in self.internal_pin_name_to_idx and input_id in self.internal_pin_name_to_idx:
                            ...
                            # edge label:[CN][CN][CN][CN][CN][CN][CN] label
    ...
    def _add_gate_pin_edges(self):
        ...
        for pin_id, pin_info in internal_pins.items():
            ...
            if comp_name in components and comp_name in self.gate_name_to_idx and pin_id in self.internal_pin_name_to_idx:
                ...
                # edgefeature: [cell_type_id, pin_type_id]
        ...
        if edges:
            ...
            print(f"  ✓ Gate-Pinedge: {len(edges)} entries")
    ...
    def _read_routing_labels(self):
        """[CN]route_data_extract.csv[CN][CN] read corresponding to[CN][CN][CN]routing[CN][CN]global label
        
         readrouting[CN][CN][CN][CN][CN][CN][CN][CN][CN]global label:
        1. utilization: [CN][CN][CN][CN][CN]
        2. hpwl: [CN][CN][CN][CN][CN]
        3. total_wire_length: [CN][CN][CN]
        4. total_vias: [CN][CN][CN][CN]
        
        [CN][CN] label[CN][CN][CN]routing[CN][CN][CN][CN][CN][CN][CN][CN][CN][CN]feature
        
        Returns:
            tuple: (utilization, hpwl, total_wire_length, total_vias)
        """
        try:
            # [CN]DEFextract design name from file path([CN][CN]_route.def[CN][CN])
            if self.def_file_path:
...
# ============================================================================
# GRAPH VALIDATOR SECTION - [CN][CN][CN][CN]region([CN][CN]B[CN][CN][CN][CN][CN][CN][CN])
# ============================================================================

class GraphValidator:
    """[CN][CN][CN][CN] - [CN][CN][CN][CN]graph's[CN][CN][CN][CN][CN][CN][CN]"""
    
    @staticmethod
    def validate(hetero_data: HeteroData) -> bool:
        """[CN][CN][CN][CN][CN] data[CN][CN][CN][CN]"""
        print("[SEARCH] [CN][CN]D[CN][CN][CN][CN] data")
        ...
        try:
            ...
            for node_type in expected_node_types:
                if node_type not in hetero_data.node_types:
                    print(f"[X] [CN][CN]nodetype: {node_type}")
                    return False
                
                if hetero_data[node_type].x is None:
                    print(f"[X] nodetype {node_type} [CN][CN]feature[CN][CN]")
                    return False
                
                print(f"  ✓ {node_type}: {hetero_data[node_type].x.shape[0]} node, feature dimension: {hetero_data[node_type].x.shape[1]}")
            ...
            for edge_type in required_edge_types:
                if edge_type in hetero_data.edge_types:
                    ...
                    print(f"  ✓ {edge_type}: {edge_index.shape[1]} entriesedge, feature dimension: {edge_attr.shape[1] if edge_attr is not None else 0}")
                else:
                    print(f"[X] [CN][CN][CN][CN][CN]edgetype: {edge_type}")
                    return False
            ...
            for edge_type in optional_edge_types:
                if edge_type in hetero_data.edge_types:
                    ...
                    print(f"  ℹ️  [CN][CN]edgetype {edge_type} [CN][CN][CN]([CN][CN][CN][CN][CN][CN] connection)")
            
            # [CN][CN]globalfeature
            if hasattr(hetero_data, 'global_features'):
                print(f"  ✓ globalfeature: {hetero_data.global_features.shape}")
            
            # [CN][CN][CN][CN][CN][CN]
            if hasattr(hetero_data, 'die_coordinates'):
                print(f"  ✓ [CN][CN][CN][CN]: {hetero_data.die_coordinates}")
            
            print("[OK] D[CN][CN][CN][CN][CN][CN][CN][CN]")
            return True
            
        except Exception as e:
            print(f"[X] [CN][CN]failed: {e}")
            return False

# ============================================================================
# MAIN CONVERSION FUNCTION - [CN] convert[CN][CN]region
# ============================================================================

def convert_def_to_d_heterograph(def_file_path: str, output_path: str = None) -> str:
    """[CN]routing[CN][CN]DEF[CN][CN] convert[CN]D[CN][CN][CN][CN]
    
    [CN][CN][CN][CN] gate processrouting[CN][CN][CN]DEF[CN][CN],generate containsroutinginformation[CN]D[CN][CN][CN][CN]:
    -  parseroutinginformation, calculatewire_length[CN]via_count
    - edge label[CN][CN][wire_length, via_count]
    - global label[CN][CN]routing[CN][CN][CN][CN][CN][CN][utilization, hpwl, total_wire_length, total_vias]
    
    Args:
        def_file_path: routing[CN][CN]DEF[CN][CN][CN][CN]
        output_path:  output[CN][CN][CN][CN]([CN][CN])
    
    Returns:
         output[CN][CN][CN][CN]
    """
    try:
        print(f"[START] [CN][CN] convertDEF[CN][CN][CN]D[CN][CN][CN][CN]: {def_file_path}")
        
        # 1.  parseDEF[CN][CN]
        parser = DEFParser(def_file_path)
        def_data = parser.parse()
        print(f"[OK] DEF[CN][CN] parsecompleted: {def_data['design_name']}")
        
        # 2.  buildD[CN][CN][CN][CN]
        builder = DHeteroGraphBuilder(def_data, def_file_path)
        hetero_data = builder.build()
        
        # 3. [CN][CN][CN] data
        if not GraphValidator.validate(hetero_data):
            raise ValueError("[CN] data[CN][CN]failed")
        ...
        print(f"[SAVE] D[CN][CN][CN][CN][CN]save: {output_path}")
        
        # 5. [CN][CN]statisticsinformation
        print("\n[CHART] D[CN]statisticsinformation:")
        print(f"  • Gatenode: {hetero_data['gate'].x.shape[0]} ")
        print(f"  • IO_Pinnode: {hetero_data['io_pin'].x.shape[0]} ")
        print(f"  • Pinnode: {hetero_data['pin'].x.shape[0]} ")
        
        total_edges = 0
        for edge_type in hetero_data.edge_types:
            edge_count = hetero_data[edge_type].edge_index.shape[1]
            total_edges += edge_count
            print(f"  • {edge_type}edge: {edge_count} entries")
        
        print(f"  • [CN]edge[CN]: {total_edges} entries")
...
# ============================================================================
# COMMAND LINE INTERFACE - [CN][CN][CN][CN][CN]region
# ============================================================================

def main():
    """[CN][CN][CN]Main function"""
    parser = argparse.ArgumentParser(
        description='Routing[CN][CN]D[CN][CN][CN][CN]generate[CN] - [CN]routing[CN][CN]DEF[CN][CN] convert[CN]Pin-Pin connection[CN][CN][CN][CN]',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
[CN][CN][CN][CN]:
  python RD_heterograph_generator.py input_route.def
  python RD_heterograph_generator.py input_route.def -o output.pt
  python RD_heterograph_generator.py /path/to/route_design.def -o /path/to/output.pt

Routing[CN][CN] convert[CN][CN]:
  • [CN][CN]netnode[CN]IO_Pin-Net,Pin-Netedge
  •  buildIO_Pin-Pin[CN]Pin-Pinedge,edge label[CN][CN][wire_length, via_count]
  • [CN][CN] outputpin connection[CN] inputpin[CN][CN]Pin-Pin convert
  •  parseroutinginformation, calculate actual[CN][CN][CN][CN][CN][CN][CN]
  • global label[CN][CN]routing[CN][CN][CN][CN][CN][CN][utilization, hpwl, total_wire_length, total_vias]
        """
    )
    
    parser.add_argument('def_file', help=' input[CN]routing[CN][CN]DEF[CN][CN][CN][CN]')
    parser.add_argument('-o', '--output', help=' output[CN].pt[CN][CN][CN][CN]([CN][CN])')
